...
def _coerce_numeric_metric_value(value: Any) -> float | None:
    """Return a scalar numeric metric value, or None for non-scalar metadata."""
...
def summarize_reward_extra(values_by_key: dict[str, Any], prefix: str) -> dict[str, float]:
    """Convert numeric reward extra info into scalar metrics with a configurable prefix."""
    ...
    for key, raw_values in values_by_key.items():
        ...
        for value in array.reshape(-1).tolist():
            if _is_missing_metric_value(value):
                continue
            numeric_value = _coerce_numeric_metric_value(value)
        ...
        if numeric_vals:
            ...
            if len(numeric_vals) == observed_count:
